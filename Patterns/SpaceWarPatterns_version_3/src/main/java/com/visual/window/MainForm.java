...
public class MainForm extends javax.swing.JFrame implements Observer {
    ...
    public void processMessage(FormMessages event){
        switch (event){
            ...
            case NewGameClick:
                Invoker.getInstance().addCommand(new NewGameCommand(controller));
                break;
            case ExitClick:
                Invoker.getInstance().addCommand(new ExitCommand(controller));
                break;
            case ContinueClick:
                Invoker.getInstance().addCommand(new ContinueCommand(controller));
                break;
            case SpacePress:
                Invoker.getInstance().addCommand(new SpacePressCommand(controller));
                break;
            case F1Press:
                Invoker.getInstance().addCommand(new ShowStatisticCommand(controller));
                break;
            case NPress:
                Invoker.getInstance().addCommand(new NewGameCommand(controller));
                break;
